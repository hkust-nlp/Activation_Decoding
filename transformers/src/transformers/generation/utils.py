        ...
        **model_kwargs,
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                if decoding_strategy == 'subject':
                ...
                elif decoding_strategy == 'tendency':
                    ...
                    logits = logits + alpha * diff
                elif decoding_strategy == 'entropy_inverse':
                    # compute entropy of the info layer
                    ...
                    # logits = logits + alpha*(1.0/entropy)
                    ...
                    logits = logits + alpha * ema_data[:, -1]

                elif decoding_strategy == 'majority_voting':
                    # compute entropy of the info layer
                    ...
                    # 
                    majority_voting = (info_layer_probs > mj_threshold).sum(dim=1).unsqueeze(0).to(torch.float16)
                    
                    majority_voting = majority_voting.scatter(1, index_nontop.unsqueeze(0), float("Inf"))
                    # mask the infrequent tokens
                    
                    if alpha!=0:
                        logits = logits + alpha*(majority_voting)
                    else:
                        logits=logits                   
